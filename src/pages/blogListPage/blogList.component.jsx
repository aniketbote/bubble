...
const BlogListPage = () => {
    ...
    useEffect(()=>{
        fetch('https://****cs78h5.execute-api.us-east-1.amazonaws.com/v1/get_latest_blog?start='+start,{
            method:'GET',
            headers: {
    ...
    const get_search_results = ()=>{
        if(searchString!==''){
            setSearched(true);
            setBlogs([]);
            fetch('https://****cs78h5.execute-api.us-east-1.amazonaws.com/v1/search_blog?search_string='+searchString,{
                method:'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin':'*'
