...
const BlogPage = ()=>{
    ...
    useEffect(()=>{
        fetch('https://****cs78h5.execute-api.us-east-1.amazonaws.com/v1/get_blog',{
            method:'POST',
            headers: {
    ...
    const handleDelete = ()=>{
        ...
        if(window.confirm('Are you sure you want to delete this blog?')){
            fetch('https://****cs78h5.execute-api.us-east-1.amazonaws.com/v1/delete',{
                method:'POST', 
                headers: {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin':'*'
     ...
     if(data.deleted){
         ...
         <ul>
             ...
             <li> Moderator found this content inappropriate.</li>
        ...
        <div className='content-column'>
            ...
            {user_id===data.user_id?
                ...
                <div style={{paddingLeft:'10px',flexGrow:1}}>
                    ...
                    <p className='time-text'> by <span className='user-text'>{data.username}</span> {timeDifference(data.timestamp)}.{data.edited?<span> (edited  {timeDifference(data.edited_timestamp)}). </span>:null}</p>       
                </div>
                ...
                <Paper style={{padding:'20px',flexGrow:1}} elevation={4}>
                    ...
                    <BlogLikes voteDisable={data.user_id===user_id} blog_id={data.blog_id} vote_count={data.upvotes}/>
